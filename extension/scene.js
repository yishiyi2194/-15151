...
function cleanPrompt(value) {
    ...
    return value
        .replace(/```(?:paintai|绘图|文生图|image)?/gi, '')
        .replace(/```/g, '')
        .replace(/^\s*[-*]\s+/, '')
        .replace(/\s+/g, ' ')
        .trim()
...
/**
 * Extracts only explicit image markers. Ordinary prose is intentionally ignored
 * so enabling automatic generation cannot silently spend image quota.
 */
export function extractImagePrompts(text, maxPrompts = DEFAULT_MAX_PROMPTS) {
    ...
    for (const pattern of patterns) {
        for (const match of text.matchAll(pattern)) matches.push({ index: match.index ?? 0, value: match[1] });
    }
...
export function scenePromptKey(chat, messageId, prompt) {
    return `${chat || 'no-chat'}:${messageId}:${prompt}`;
}
